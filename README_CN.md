...
* `ida_mcp/` - 核心 IDA 能力层、gateway、proxy、lifecycle、tools、resources
* `ide/` - 独立的 PySide6 桌面 IDE，负责安装、配置、状态检查以及多 Agent 审计工作台
...
`open_in_ida` 是 proxy 侧的生命周期工具。它会使用 `config.conf` 中的 `ida_path` 解析 IDA 可执行文件，并通过子进程环境变量 `IDA_MCP_AUTO_START=1` 和预留好的 `IDA_MCP_PORT` 请求插件自动启动。是否追加 `-A` 由 `config.conf` 中的 `open_in_ida_autonomous` 控制，默认值仍然是 `true`。

`open_in_ida` 会通过 `config.conf` 解析 IDA 可执行文件。文件复制是可选的：只有配置了 `config.conf` 中的 `open_in_ida_bundle_dir` 时，`open_in_ida` 才会在该根目录下创建时间戳目录并复制目标文件。若目标旁边已经存在匹配的 `.i64` 或 `.idb`，它也会一起复制，并直接启动数据库路径，这样 IDA 会直接进入现有界面，而不会再次弹出 loader / 架构等默认打开确认框。未配置 staging 时，`open_in_ida` 会直接打开原始路径；若存在匹配数据库，也会优先打开数据库。

由于默认 `open_in_ida_autonomous=true`，IDA 会以 batch/autonomous 模式启动。它更适合无人值守自动化，也可能减少部分交互确认流程，但这不等价于正常手工逆向时的 GUI 启动：交互式对话框可能被抑制，依赖人工确认的 loader / 插件 / UI 流程可能表现不同。

如果你要“先自动化，再人工接管”，推荐两阶段流程：

1. 保持 `open_in_ida_autonomous=true`，再用 `open_in_ida(...)` 跑自动流程。
2. 保存生成的 `.i64/.idb`。
3. 把 `open_in_ida_autonomous=false` 后，再用 `open_in_ida(...)` 重新打开这个数据库，进入正常人工操作。
...
* gateway 对客户端暴露的访问地址仍然由 `http_host` 派生；实例侧监听地址由 `ida_host` 配置
* `ida_python` 会记录安装时选中的 IDA 侧 Python，方便后续查看当前配置环境。gateway/proxy 子进程在运行时也会使用 `ida_python`
* `open_in_ida` 不再接受 `ida_path` 工具参数；请通过 `config.conf` 配置 IDA 路径
* `open_in_ida` 会为启动出来的 IDA 子进程设置 `IDA_MCP_AUTO_START=1` 和 `IDA_MCP_PORT=<reserved_port>`
* `open_in_ida` 不再接收 `autonomous` 工具参数；请改为通过 `config.conf` 里的 `open_in_ida_autonomous` 控制
* `open_in_ida_autonomous` 默认值是 `true`，所以 `open_in_ida` 会默认追加 `-A`；配置关闭后则不会追加
* `-A` 会让 IDA 进入 batch/autonomous 启动模式；适合“先自动化保存数据库，再把 `open_in_ida_autonomous=false` 后重新打开做人工分析”
* 启用 `-A` 后，确认对话框以及部分 loader / 插件 / UI 流程可能被抑制，行为会和正常 GUI 启动不同
* `open_in_ida` 只有在配置了 `open_in_ida_bundle_dir` 时才会复制文件
